...
```
We can then use this in a custom gym environment as follows:
```python
import numpy as np
import gym
from gym.spaces import Box
...
class SimpleEnv(gym.Env):
    def __init__(self):
        ...
        # TODO: add action space (in the next section)

    def step(self, action_index):
        state = # TODO: get the state
        # TODO:
...
To make this a little easier, `zero_ad_rl` uses `StateBuilder` and `ActionBuilder` classes. These classes enable you to separate the state/action representation from the rest of the logic of the environment (which may include configurations to load and/or levels in the case of curriculum learning). Using a `StateBuilder`, we could implement the above logic as follows (without all the extra placeholders for code that needs to be added in the future!):
...
class AttackRetreat(ActionBuilder):
    def __init__(self):
        space = Discrete(2)  # TODO: EXPLAIN THIS
        super().__init__(space)  
...
Now that we have defined our custom state and action spaces, we can create our custom OpenAI gym environment that uses them! There is a generic environment in `zero_ad_rl` that we can now use to create the OpenAI Gym environment using our state and action builders:

```python
from zero_ad_rl.env.base import ZeroADEnv

address = 'http://127.0.0.1:6000'
scenario_config = # TODO
env = ZeroADEnv(address, scenario_config, action_builder, state_builder)
```

Now, we can use `env` with any OpenAI Gym-compatible RL algorithms or frameworks!

## Training an agent!
One example RL framework that we can use with our new environment is RLlib. In order to configure our 
